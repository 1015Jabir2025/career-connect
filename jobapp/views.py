...
def home_view(request):
    ...
    # Django 4+ removed request.is_ajax(); use header check instead
...
def job_list_View(request):
    ...
    # If user is authenticated and is an employee, sort jobs so matches come first
    if request.user.is_authenticated and hasattr(request.user, 'role') and request.user.role == 'employee':
        ...
        user_subcategory = request.user.subcategory
        job_list = sorted(
            job_list,
            key=lambda job: not (
                (job.category and job.category == user_category) or (job.sub_category and job.sub_category == user_subcategory)
            )
        )
